...
\SSpsetQ{
    Tan and Wen are writing questions for CCCC at a rate of 1 per minute. Immediately after Tan writes a question, Wen eats his paper with probability \( \frac 17 \), so that Tan must restart.\\

    After an extremely long time (assume infinite), Tony Wang walks in. What is the expected number of questions Tony Wang sees written on Tan's paper?
}\bigskip 
